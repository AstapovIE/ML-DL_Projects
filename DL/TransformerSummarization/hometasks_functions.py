...
def plot_attention_grid(attn_weights, input_tokens, output_file, num_first_tokens=10):
    ...
    attn_weights: Tensor (heads, seq_len, seq_len) - attention-матрицы
    input_tokens: List[str] - список слов входного текста
    """
...
def task3(model, test_iter, vocab, num_examples, device, output_file):
    ...
    for i in range(num_examples):
        ...
        # Декодируем входные токены в текст
        input_tokens = [vocab.itos[idx] for idx in source_inputs[0]
                        if idx not in {vocab.stoi['<s>'],
                                       vocab.stoi['</s>'],
                                       vocab.stoi['<pad>']}]

        # Извлекаем attention-матрицы (они должны быть сохранены в _attn_probs)
        attn_weights = model.encoder._blocks[0]._self_attn._attn_probs  # Tensor (batch, heads, seq_len, seq_len)
