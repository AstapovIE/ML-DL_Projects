...
class Transformer(nn.Module):
    ...
    def generate_summary(self, source_inputs, target_inputs, source_mask, target_mask):
        with torch.no_grad():
            ...
            #         [self.decode_tensor(F.log_softmax(seq, dim=-1).argmax(dim=-1)).split("</s>")[0] + "</s>" for seq in outputs])

            # чистый вывод
            # return ([self.decode_tensor(seq) for seq in source_inputs],
            #         [self.decode_tensor(seq).split("</s>")[0].split("<s>")[1] for seq in target_inputs],
            #         [self.decode_tensor(F.log_softmax(seq, dim=-1).argmax(dim=-1)).split("</s>")[0].split("<s>")[1] for seq in
            #          outputs])
